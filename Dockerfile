...
RUN curl -Lo /usr/local/bin/cyclonedx-cli https://github.com/CycloneDX/cyclonedx-cli/releases/download/v0.27.1/cyclonedx-linux-x64 \
    && chmod +x /usr/local/bin/cyclonedx-cli


# Setup Entrypoint Script
COPY entrypoint.sh /usr/local/bin/entrypoint.sh
RUN chmod +x /usr/local/bin/entrypoint.sh && chown pqcuser:pqcuser /usr/local/bin/entrypoint.sh
